# coding=utf-8
# Microsoft Research Paraphrase Corpus Classifier, A BERT Finetuning runner
...
class MrpcProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "msr_paraphrase_train.txt")))
...
def evaluate(processor, tokenizer, model, device, data_dir, output_dir):
    label_list = processor.get_labels()
    num_labels = len(label_list)
    eval_batch_size = 8
...
def train_main():
    ...
    model = BertForSequenceClassification.from_pretrained("./bert_models/bert-base-uncased.tar.gz",
                                                          num_labels=num_labels)
    model.to(device)

    # fine tuning 训练
    fine_tuning_train(processor, tokenizer, model, device, data_dir)
